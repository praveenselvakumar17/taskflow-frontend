...
const TaskModel = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (taskToEdit) {
      // ✅ FIX: Normalize completed to "Yes"/"No" for the form
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        // ✅ FIX: Convert "Yes"/"No" to boolean before sending
        const payload = {
        ...
        // Call onSave with boolean value
        onSave?.(payload);
        onClose();
      } catch (err) {
        console.error(err);
      ...
      <div className="bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center border border-purple-100 rounded-lg px-3 py-2.5 focus-within:ring-2 focus-within:ring-purple-500 transition-all">
              <input
                type="text"
                name="title"
                required
